...
sub_grid_size = 0.04
ply_folder = join(dirname(dataset_path), 'S3DIS_ply')
os.mkdir(ply_folder) if not exists(ply_folder) else None

original_pc_folder = join(ply_folder, 'original')
os.mkdir(original_pc_folder) if not exists(original_pc_folder) else None

sub_pc_folder = join(ply_folder, 'sub_{:.3f}'.format(sub_grid_size))
