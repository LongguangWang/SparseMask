...
class Plot:
    ...
    def draw_pc_sem_ins(pc_xyz, pc_sem_ins, plot_colors=None):
        ...
        for id, semins in enumerate(sem_ins_labels):
            ...
            xmin = valid_xyz[:, 0].min()
            xmax = valid_xyz[:, 0].max()
            ymin = valid_xyz[:, 1].min()
            ymax = valid_xyz[:, 1].max()
            zmin = valid_xyz[:, 2].min()
            zmax = valid_xyz[:, 2].max()
            sem_ins_bbox.append([
                [xmin, ymin, zmin],
                [xmax, ymax, zmax],
                [min(tp[0], 1.), min(tp[1], 1.), min(tp[2], 1.)]
            ])
...
class checkpoint():
    def __init__(self, args):
        ...
        if args.dataset == 'S3DIS':
            if not os.path.exists(args.logs_dir + '/' + args.dataset):
                os.mkdir(args.logs_dir + '/' + args.dataset)
            path = './' + args.logs_dir + '/' + args.dataset + '/Area' + str(args.idx_test_area)
        elif args.dataset == 'SemanticKITTI':
            path = './' + args.logs_dir + '/' + args.dataset

        if not os.path.exists(path):
            os.mkdir(path)

        open_type = 'a' if os.path.exists(path + '/log.txt') else 'w'
        self.log_file = open(path + '/log.txt', open_type)
        with open(path + '/log.txt', open_type) as f:
            f.write(now + '\n\n')
            f.flush()
