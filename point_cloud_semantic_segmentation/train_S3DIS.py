...
def evaluate(args, model, loader, criterion, log_file):
    ...
    with torch.no_grad():
        i = 0
        for _, (points, labels, neighbor_idx, _, _) in enumerate(loader):
            i += 1
...
def train(args, log_file):
    # dataloaders
    train_dataset = dataset_S3DIS(args, training=True)
    ...
    # resume
    if args.resume > 0:
        resume_file = args.logs_dir + '/' + args.dataset + '/Area' + str(args.idx_test_area) + '/epoch_' + \
                      str(args.resume) + '.pth'
    ...
    for _, (points, labels, neighbor_idx) in enumerate(train_loader):
        ...
        # calculate loss
        loss = criterion(scores, labels) + 0.04 * sparsity.mean()
        ...
        if i % args.train_steps == 0:
            ...
            if epoch % args.save_freq == 0:
                torch.save(
                    ...
                    args.logs_dir + '/' + args.dataset + '/Area' + str(args.idx_test_area) + '/epoch_' + str(epoch) + '.pth'
                )
